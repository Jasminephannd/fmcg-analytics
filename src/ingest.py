...
def download_dataset():
    try:
        ...
        logger.info(f"downloading {DATASET} -> {config.BRONZE_DIR}")
        api.dataset_download_files(DATASET, path=str(config.BRONZE_DIR), unzip=True)

        csvs = sorted(config.BRONZE_DIR.glob("*.csv"))
        logger.info(f"downloaded {len(csvs)} CSV files:")
        for f in csvs:
            logger.info(f"  {f.name:30s} {f.stat().st_size / 1_048_576:6.1f} MB")
    except Exception:
