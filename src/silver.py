...
def clean_transactions(engine):
    ...
    for col in ["sales_value", "retail_disc", "coupon_disc", "coupon_match_disc"]:
        df[col] = df[col].astype(float)                       # money columns
    df["retail_disc"] = df["retail_disc"].round(2)           # round retail_disc to 2 dp (snaps float noise like 2.2e-16 to 0.00)
    ...
    print(f"  dropped {before - len(df):,} zero-qty / zero-sales lines")
    return df
...
def clean_coupon(engine):
    ...
    print(f"  dropped {before - len(df):,} duplicate coupon rows")
    return df
...
def main():
    ...
    for name, clean_fn in tables.items():
        ...
        print(f"silver.{name:16s} {len(df):>10,} rows")
